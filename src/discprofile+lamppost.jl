...
function plot_data(dx_at,dx_ab,dx_bt,dx_bb,dx_ct,dx_cb)
    ...
    plot!(r,dx_at,fillrange=dx_ab, label=L"\dot{M} / \dot{M}_\textrm{Edd} = 0.1", color = :orange, fillcolor = :tan1)
    x=zeros(30)
...
m=KerrMetric(a=0.9)
d1_at,d1_ab,d1_bt,d1_bb,d1_ct,d1_cb=disk_height(m)
...
scatter!([pos],[height],label=L"\textrm{h=3 r_g}")
scatter!([pos],[6.0],label=L"\textrm{h=6 r_g}")
scatter!([pos],[12.0],label=L"\textrm{h=12 r_g}")
plot_horizon!(m, color = :black, label = L"\textrm{Horizon}")
